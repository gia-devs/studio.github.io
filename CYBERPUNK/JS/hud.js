...
document.querySelectorAll(".abc2.keyboard button").forEach((button) => {
    ...
    const flash = () => {
        ...
        document.querySelectorAll(".abc2.keyboard button.is-pressed").forEach((b) => {
            if (b !== button) b.classList.remove("is-pressed");
        });
        button.classList.add("is-pressed");
        sound.play("key");
        pressTimer = window.setTimeout(() => button.classList.remove("is-pressed"), pressMs);
    };

    const typeKey = () => {
        const key = button.dataset.key;
        if (!key || !glyphScreen) return;

        // Inserta el carácter en la posición del cursor
        glyphScreen.focus();
        document.execCommand("insertText", false, key);

        // Alternativa más moderna (si prefieres):
...
// Opcional: permitir borrar con Backspace cuando el foco está en la pantalla
glyphScreen?.addEventListener("keydown", (e) => {
    if (e.key === "Backspace") {
        // comportamiento normal de contenteditable
        return;
    }
    // Si quieres bloquear la escritura real del teclado físico y solo usar los botones:
    // e.preventDefault();
});
